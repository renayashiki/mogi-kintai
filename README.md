...
```
> 管理システムへのログインが可能です。管理者でログイン後、申請一覧画面にて「西 伶奈」「山田 太郎」「山田 花子」の承認待ち申請を確認できます。
...
``` bash
 ...
 'password' => 'password123'
```
> 2023年6月の勤怠一覧において、見本同様に4日、7日、17日、25日が休日（空白）として表示されます。

**一般ユーザー：西 玲奈（管理者画面再現用）**
``` bash
 'email' => 'reina.admin@coachtech.com',
 'password' => 'password123'
```
> 管理画面見本の「スタッフ一覧」では **西 怜奈** と表示されますが、詳細ボタン押下後の「スタッフ別勤怠一覧」では **西 玲奈** と表示される仕様および「同月で空白日が違う」を見本通りに再現した内容が確認できるテスト用アカウントです。
...
``` bash
 ...
 'password' => 'password123'
```
> ログイン直後の画面: ステータスは 「勤務外」のため、すべてのステータス遷移を確認することが出来ます。また、休憩が2回ある状況等も直近3日間で見れるようになっています。

**各ステータス表示の即時確認用**
特定のステータス表示を即座に確認したい場合は、以下の各アカウントをご利用ください。
| 確認したいステータス | 名前 | メールアドレス | ログイン時の状態 |
| :--- | :--- | :--- | :--- |
| **勤務外** | 山田 太郎 | `taro.y@coachtech.com` | ログイン直後から「勤務外」 |
| **出勤中** | 増田 一世 | `issei.m@coachtech.com` | ログイン直後から「出勤中」 |
| **休憩中** | 秋田 朋美 | `tomomi.a@coachtech.com` | ログイン直後から「休憩中」 |
| **退勤済** | 中西 教夫 | `norio.n@coachtech.com` | ログイン直後から「退勤済」 |
